...
def _nhidcl_due_at(value: Any) -> str | None:
    ...
    for date_format in ("%a, %m/%d/%Y - %H:%M", "%m/%d/%Y - %H:%M", "%d-%m-%Y %H:%M:%S"):
        try:
            parsed = datetime.strptime(text, date_format).replace(tzinfo=IST)
...
def _nhai_lifecycle(status: str, title: str) -> str:
    ...
    if "under construction" in combined:
        return "construction"
    if "cc issued" in combined or "pcc issued" in combined or "completed" in combined:
        return "completed"
        ...
        contracts.append(
            {
                ...
                "state_code": code,
                "agency": "MoRTH",
                "lifecycle": _nhai_lifecycle(status, title),
...
def normalise_nhidcl(rows: Iterable[dict[str, Any]], retrieved_at: str) -> list[dict[str, Any]]:
    ...
    for key, (_, row, code, tender_id, due_at) in selected.items():
        ...
        fingerprint = _notice_fingerprint(title, code)
        if due_at:
            status = "active tender notice" if _parse_instant(due_at) >= retrieved else "bid due date passed"
        else:
            status = "tender notice; bid due date unavailable"
        contracts.append(
            {
