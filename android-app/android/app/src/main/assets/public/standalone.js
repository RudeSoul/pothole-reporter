...
(() => {
  ...
  // Restored with the routing repair: these were lost in the v1.38 merge, leaving
  // routeOfficer calling functions that did not exist. Every complaint route threw.
  ...
  // Restored with the v1.38 merge repair: these definitions were dropped while their
  // callers stayed, so the repair, condition and tender paths threw on first use.
  ...
  async function createReport(fd, driveMode) {
    ...
    const rec = {
      ...
      body_name: centralJurisdiction && centralJurisdiction.road_ownership === "municipal"
        ? centralJurisdiction.town || null : route.authority_name || null,
  ...
  const __pure = {
                   ACCEPTED_REPORT_STATUSES, ALLOWED_DETAILS, ALLOWED_MODELS,
                   ANDHRA_PRADESH_ROUTING_ENVELOPE, ANDHRA_PRADESH_STATE_AUTHORITY,
                   ANDHRA_PRADESH_STATE_GEOMETRY_SHA256, ASSESSMENT_RE, ASSESS_SCHEMA,
                   ...
                   validateHighwayManifest, validateHighwayTile, validateKarnatakaBodies,
                   validateKarnatakaStatePayload, validateKeralaPayload,
                   validateMadhyaPradeshPayload, validateMajorCityPayload,
                   validateMunicipalCityPayload, validateOdishaPayload,
                   validateOfficialHandoffRegistry, validatePackEnvelope,
                   validatePinnedStatewidePayload, validatePunjabPayload,
                   validateRajasthanPayload, validateRoadAgreementManifest,
                   validateRoadAgreementPack, validateRoadNoticeManifest,
                   validateRoadNoticePack, validateRoutingPack, validateStatePackManifest,
